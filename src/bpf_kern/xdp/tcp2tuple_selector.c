...
#define XDP_MAX_TCP2TUPLE_NUM 50000
#define XDP_TCP2TUPLE_MAP_PATH "/sys/fs/bpf/eMPTCP/xdp_tcp2tuple_map"
...
BPF_TABLE_PINNED("hash", struct tcp2tuple, action_chain_id_t, xdp_tcp2tuple_map, XDP_MAX_TCP2TUPLE_NUM, XDP_TCP2TUPLE_MAP_PATH);
...
int tcp2tuple_selector(struct xdp_md *ctx) 
{
    ...
    get_tcp2tuple_in(iph, &tcp2t);

#ifdef NOBCC
    ACTION_CHAIN_ID = bpf_map_lookup_elem(&xdp_tcp2tuple_map, &tcp2t);
#else
    ACTION_CHAIN_ID = xdp_tcp2tuple_map.lookup(&tcp2t);
#endif
